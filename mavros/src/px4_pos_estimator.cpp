...
void optitrack_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    {
        // Read the Drone Position from the Vrpn Package [Frame: Vicon]  (Vicon to ENU frame)
        pos_drone_mocap = Eigen::Vector3d(-msg->pose.position.z,-msg->pose.position.x,msg->pose.position.y);
        // Read the Quaternion from the Vrpn Package [Frame: Vicon[ENU]]
...
int main(int argc, char **argv)
{
    ...
    // 【订阅】optitrack估计位置
    ros::Subscriber optitrack_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/Body/pose", 1000, optitrack_cb);
